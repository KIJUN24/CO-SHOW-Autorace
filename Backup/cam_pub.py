...
class CameraRec :
    def __init__(self):
        ...
        self.light_color = ''
        self.red_lower = np.array([140, 130, 180])  
        self.red_upper = np.array([255, 255, 255])
        self.green_lower = np.array([70, 190, 110])  
        self.green_upper = np.array([90, 255,255])  
        self.orange_lower = np.array([5, 150, 155])
        self.orange_upper = np.array([40, 255, 200])
        self.blue_lower = np.array([85, 175, 110])
        self.blue_upper = np.array([110, 255, 255])

        # point reset
        self.left_point_x = 0
        self.right_point_x = 0
        self.vertical_value = 100
        self.center_point = 320

        # roi variable
        self.roi_line_up = 300
        # self.roi_line_down = 400
        self.roi_line = 300
        ...
        # stopline variable
        self.stopline_val = 10000
        self.stopline_detct = False
        
        # redzone
        self.redzone_detect = False
        self.redzone_lower = np.array([160, 30, 150])
        self.redzone_upper = np.array([190, 255, 255])  
    ...
    def Img_canny(self, img):
        ...
        self.img_blur = cv2.GaussianBlur(img_gray, (27, 27), 0)
        img_edge = cv2.Canny(np.uint8(self.img_blur), 10, 30)
    ...
    def Img_stopline(self):
        img_roi = self.img_blur.copy()[self.roi_stopline_x1:self.roi_stopline_x2, self.roi_stopline_y1:]
