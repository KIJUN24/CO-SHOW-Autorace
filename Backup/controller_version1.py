...
class Controller :
    def __init__(self):
        ...
        ##################  Flag  ##################
        self.flag_traffic_light = False
        self.flag_dynamic = False
        self.flag_static = False
        self.flag_labacorn = False
        self.flag_stopline = False
    ...
    def Drive_labacorn(self):
        ...
        for idx, dist in enumerate(self.laba_objects):
            ...
            if len(self.left_labacorn_deg) >= 1 and len(self.right_labacorn_deg) >= 1:
                ...
                print(f"left_pair:{self.left_pair}")
                print(f"left_close_dist:{self.left_close_dist}")
                print(f"first_deg:{first_deg}")
    ...
    def Object_detect(self):
        self.Lidar_range_change(170,190,0.7)
    ...
    def Parking_hard_detect(self):
        ...
        if self.flag_parking_current_state == 1:
        ...
        elif self.flag_parking_current_state == 3 and self.num_object == 0:
    ...
    def Parking_hard_1(self):
        self.Lidar_range_change(1,35,0.7)
        start_time = rospy.get_time()
        end_time = rospy.get_time()
    ...
    def Parking_cal_detect(self):
        ...
        if self.flag_parking_current_state == 1:
            ...
            self.Drive_stop()
            self.flag_parking_current_state = 5
    ...
    def Parking_cal_2(self):
        ...
        for idx, val in enumerate(self.parking_objects):
            ...
            if parking_dist < 0.33:
                parking_cal_list_2.append(parking_dist)
        if len(parking_cal_list_2) == 0:
            self.Drive_controller(-0.2, 3.4)
        else:
            self.Drive_controller(0,0)
            self.flag_parking_cal_step = 2
        print(f"Parking_cal_2:{parking_cal_list_2}")
    ...
    def Parking_cal(self):
        if self.flag_parking_current_state == 5 and self.flag_parking_cal_step == 0:
            self.Parking_cal_1()
            ...
            self.Getout_cal_2()
        elif self.flag_parking_cal_step == 4 and self.flag_line_tracer == True:
    ...
    def Timer_CB(self, event):
        ...
        self.Line_tracer(self.input_speed)
        ##########################   scenario   #########################
        # Line Tracer -> Labacorn Drive -> Static Avoid -> Line Tracer  #
        ########### Line Tracer -> Static Avoid -> Stop line  ###########
        #################################################################
        ...
        # # Object Detect
        # elif self.flag_static and self.flag_driving_mode == 1:
        #     self.Object_detect()
        # # Parking
        # elif self.flag_driving_mode == 6 and self.light_color == "Blue":
        ...
        # print(self.flag_labacorn)
        # print(self.stopline_check)
        ...
        # print(self.flag_parking_mode)
        # print(self.flag_line_tracer)
        ...
        #     while self.flag_parking_mode == True:
        #         self.flag_line_tracer = False
        #         if self.flag_parking_current_state != 5:
        #             self.Line_tracer(0.2)
        #             self.Parking_cal_detect()
        #         elif self.flag_parking_current_state == 5:
        #             self.Parking_cal()
        ...
        # print(f"color:{self.light_color},  state:{self.flag_parking_current_state}")
