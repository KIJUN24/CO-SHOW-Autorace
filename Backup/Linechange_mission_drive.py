...
class Controller :
    ...
    def Timer_CB(self, event):
        ...
        #     self.Line_tracer(0.3)

        # if self.flag_driving_mode == 2:
        #     self.Line_tracer(self.input_speed)

        # if len(self.ar_mark) != 0:
        #     self.flag_driving_mode = 1
        
        self.Line_tracer(self.input_speed)
        if self.flag_driving_mode == 5:
            self.Line_Change_Value()   
        if self.flag_driving_mode == 6:
            print(self.mark_id)
            self.Traffic_Line_Change()
        if self.flag_driving_mode == 7:
            self.Line_tracer(self.input_speed)
        print(self.flag_driving_mode)
