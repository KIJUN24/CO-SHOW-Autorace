...
class Controller :
    ...
    def Object_type_decision(self):
        ...
        for idx, dist in enumerate(self.objects):
            pre_obs_deg = dist.deg
            pre_obs_deg_list.append(pre_obs_deg)
            pre_obs_deg_value = pre_obs_deg_list[0][-1]
        ...
        while (end_time - start_time) <= 1.5:
            for idx, dist in enumerate(self.objects):
                cur_obs_deg = dist.deg
                cur_obs_deg_list.append(cur_obs_deg)
                cur_obs_deg_value = cur_obs_deg_list[0][-1]
            end_time = rospy.get_time()
                # print(cur_obs_deg)
        
        # if cur_obs_deg_list[0] - pre_obs_deg_list[0] >= 6:
        #     rospy.loginfo("dynamic_obs")
        #     self.dynamic_flag = True
        # elif cur_obs_deg_list[0] - pre_obs_deg_list[0] <= 3:
        #     rospy.loginfo("static_obs")
        #     self.static_flag = True
        print(f"pre_obs_deg_value:{pre_obs_deg_value}, cur_obs_deg_value:{cur_obs_deg_value}")

        if cur_obs_deg_value - pre_obs_deg_value >= 6:
            rospy.loginfo("dynamic_obs")
            self.dynamic_flag = True
        elif cur_obs_deg_value - pre_obs_deg_value <= 3:
            rospy.loginfo("static_obs")
            self.static_flag = True
    ...
    def Parking_hard_1(self):
        ...
        start_time = rospy.get_time()
        end_time = rospy.get_time()
        while end_time - start_time <= 2.55:
    ...
    def Timer_CB(self, event):
        ...
        if self.flag_driving_mode == 4: #로터리,차단기
            self.Lidar_range_change(110, 250, 0.8)
        ...
        if self.flag_driving_mode == 5 and self.num_object == 2: #터널
            while True:
                ...
                if len(self.mark_dist) == True:
                    self.Drive_stop()
                    print("UPUP")
                    self.flag_driving_mode = 6
                    break

        # if self.flag_driving_mode == 6: #마커 인식
        #     self.Line_Change_Value()
        ...
        if self.flag_driving_mode == 6 and self.stopline_check == True: 
            self.flag_line_tracer = False
            print("Parking")
            self.Parking_hard()
